...
from ..tr069 import ftp
import datetime
from ..tr069.data import event
...
class Client:
    ...
    def _record_response(self, response: requests.Response) -> None:
        # todo:判断是否有内容
        if response.status_code == 204 or (response.status_code == 200 and (not (response.content == b''))):
            self.response_real_count += 1
    ...
    def handle_server_rpcs(self) -> int:
        ...
        while True:
            ...
            if len(self.messages)>=2:
                prerpc = self.messages[-2]
                rpc_name = soap.extract_rpc_name(prerpc.text) or "unknown"
                if rpc_name == "cwmp:Upload":
                    start_time = datetime.datetime.now().isoformat()
                    self.ftp_upload(prerpc)
                    end_time = datetime.datetime.now().isoformat()
                    commandkey = soap.extract_rpc_commandkey(prerpc.text)
                    m_upload_event = event.Event(code='M Upload',command_key=commandkey)
                    self.inform(events = (event.TransferComplete,m_upload_event))
                    count += 1
                    self.request_transfercomplete(start_time, end_time, commandkey)
                    count += 1
                    self.done()
                    count += 1
                    break
                    #self.handle_server_rpcs()
                    ...
                    if rpc.status_code == 204:
                        break
                    # todo:判断是否有内容
                    if rpc.status_code == 200 and (rpc.content == b''):
                        self.done()
                        count += 1
                        continue
    ...
    def _handle_server_rpc(self, rpc: requests.Response):
        ...
        elif rpc_name == "cwmp:Upload":
            start_time = datetime.datetime.now().isoformat()
            end_time = datetime.datetime.now().isoformat()
            self.upload_response(start_time, end_time)
    ...
    @_wrap_rpc(rpcs.make_upload_response)
    def upload_response(self, *args, **kwargs):
        return self.request(rpcs.make_upload_response(*args, **kwargs))

    @_wrap_rpc(rpcs.make_request_transfercomplete)
    def request_transfercomplete(self, *args, **kwargs):
        return self.request(rpcs.make_request_transfercomplete(*args, **kwargs))

    def ftp_upload(self,xml):
        (addr, port, file, username, password) = soap.extract_rpc_ftpinfo(xml.text)
        print(addr, port, file, username, password)
        my_ftp = ftp.MyFTP(addr, port)
        my_ftp.login(username, password)
        my_ftp.upload_file(self.device.cfgpath, file)
        my_ftp.close()
